...
module common_cmuvp
	implicit none
		! up, vp: �Z�������̗���(��ʍ��W)�Aux, uy�F�Z�������̃f�J���g����
...
module common_cmgrd
	implicit none
		! gux, guy, gvx, gvy: CIP�Ɏg���������z(du_xi/dxi, du_xi/det)
...
module common_cmuxy
	implicit none
		! uxx, uyy: �f�J���g���W�̗���(�i�q�_)�Aqbxx, qbyy: �f�J���g���W�̑|������(�i�q�_)
...
module common_cmcf
	implicit none
		! cf: �͏���R�W���Are: ���C�m���Y���Avege_el: �A���̍���
	real(8),dimension(:,:),allocatable :: cf, re, cd_veg, cd_soft_veg, theta_soft_veg, vege_el, vege_h
...
module common_cmsnu
	implicit none
		! snu: �Q���S���W��(�Z������)�Asnu_x:snu�̊i�q�_�̒l
...
module mix
	...
	integer,dimension(:,:),allocatable :: nb, flg_mix
		! e_d: �͐ϑw���Ae_thick: �ړ��w���Ae_m: �����w��
	real(8) :: e_d, e_thick, e_m
		! ddk: �e���a�T�C�Y�Atsci: �Օ����ʂ��l�����Ȃ��Ƃ��̖��������E�|����
...
contains
	...
	subroutine alloc_var1(im, jm)
		...
		allocate( cf(0:i,0:j), re(0:i,0:j), cd_veg(0:i,0:j), cd_soft_veg(0:i,0:j), theta_soft_veg(0:i,0:j), vege_el(0:i,0:j), vege_h(0:i,0:j) )
...
contains
	...
	subroutine initial_01
		...
		cf=0.d0; re=0.d0; cd_veg=0.d0; cd_soft_veg=0.d0; theta_soft_veg=0.0d0
...
382             continue
    ...
    if(j_conf == 1) then		!h101104 conf
       ...
       qu(  j_t2,j) = qu(  j_t2-jxd,j)		!h101104 conf23�㗬�[
...
!
		do i=1,nx
			...
			do j=ny-1,1,-1
				if( ijo_in(i,j)==0 .and. ijo_in(i,j+1)==0 ) then		! �ǂ���Ƃ��\�����ł͂Ȃ�
					if( hs(i,j)>hmin .or. hs(i,j+1)>hmin ) then			! ���������ۂ̎�
						if( phi(i,j+1)==1.d0 ) then							! �����ق��Ɍ����w�ȏ�̍�������
...
!     call iric_initoption(IRIC_OPTION_CANCEL, ier)
   ...
   !
   !   ��Lj_wl=1�̎������v�Z�ɗp������z��
   !   j_slope=0.... �͏��f�[�^���玩���I�Ɍv�Z����
   !   j_slope=1.... �^���遨���̎��͗^����l�͎���bh_slope�̒l

     CALL cg_iric_read_real(fid,'bh_slope', bh_slope, ier)
   !
   !   ��Lj_wl=1��j_slope=1�̏ꍇ�ɗ^������z = bh_slope
   !
   ...
   !
   !   ��Lj_upv=1��j_upv_slope=1�̏ꍇ�ɗ^������z = upv_slope
   !   �x�쑤�̌��z = upv_slope_t
   !
   ...
   !  ��Li_flow=1�̂Ƃ��̏������ʌ��z(�㗬���x��)h_slope_12t
   !
   ...
   do j=1,ny
      do i=1,nx
       ...
       ! ----- set soft vegetation condition -----
		 theta_soft_veg = ((pi / 2) - asin(0.2d0)) * ((usta(i,j) - 0.12d0)**2)
		 if (theta_soft_veg >= ((pi / 2) - asin(0.2d0))) then
			theta_soft_veg = ((pi / 2) - asin(0.2d0))
		 elseif (usta(i,j) <= 0.12d0) then
			theta_soft_veg = 0.0d0
		 end if
