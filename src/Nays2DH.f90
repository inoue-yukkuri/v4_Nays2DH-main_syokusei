...
module mix
	...
	real(8) :: e_d, e_thick, e_m
		! ddk: 各粒径サイズ、tsci: 遮蔽効果を考慮しないときの無次元限界掃流力
	...
	real(8),dimension(:,:),allocatable :: pdist_d_100, pdk0
		! e_t: 遷移層厚、dm_t: 遷移層の中央粒径、dm_m: 交換層の中央粒径、tscm: 中央粒径に対する限界無次元掃流力
	...
	real(8),dimension(:,:,:),allocatable :: qbxkc, qbykc
		! tsk: 各粒径の無次元掃流力、tsck: 各粒径の無次元限界掃流力
			...
			if( e_t(i,j)+dex(i,j)>0.d0 ) then
				...
				if( nb(i,j)==0 ) then
					...
					do k=1,nk
						...
						p_t_new(k) = p_d(i,j,nb(i,j),k)		! ダミー
...
!
		do i=1,nx
			...
			do j=ny-1,1,-1
				if( ijo_in(i,j)==0 .and. ijo_in(i,j+1)==0 ) then		! どちらとも構造物ではない
					if( hs(i,j)>hmin .or. hs(i,j+1)>hmin ) then			! 水中か水際の時
						if( phi(i,j+1)==1.d0 ) then							! 崩れるほうに交換層以上の砂がある
